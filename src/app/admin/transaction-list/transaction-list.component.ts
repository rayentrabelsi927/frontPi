...
console.log(this.transactions)
  ...
  searchTransactionsByUser(searchQuery: string) {
  if (searchQuery && searchQuery.trim() !== '') {
    const searchRole = searchQuery.toLowerCase().trim(); // Normaliser et nettoyer la recherche
    this.filteredTransactions = this.transactions.filter(transaction =>
      transaction.users.role.toLowerCase().includes(searchRole)
    );
  } else {
    this.filteredTransactions = this.transactions; // Réinitialiser avec les données d'origine
  }
